 ...
 */
@Component
public class DeleteProxyAction extends AbstractAction {
    /**
    ...
    @PostConstruct
    protected void init() {
        this.id = "configman.delete-proxy";
        this.displayName = ts.getTranslatedString("module.configman.proxies.actions.delete-proxy.name");
        this.description = ts.getTranslatedString("module.configman.proxies.actions.delete-proxy.description");
        this.order = 1000;

        setCallback((parameters) -> {
            BusinessObjectLight proxy = (BusinessObjectLight) parameters.get("proxy");
            try {
                pms.deleteProxy(proxy.getClassName(), proxy.getId());
