 ...
 */
public class FreePortsComponentForSingleMirrors extends AbstractFreePortsComponent<SingleMirrorEditor> {
    public FreePortsComponentForSingleMirrors(BusinessObjectLight businessObject, 
    ...
    @Override
    public SingleMirrorEditor getMirrorEditor(PortComponent source, PortComponent target) {
        return new SingleMirrorEditor(source, target, lytAssignedPorts, cmdUpdateFreePorts, bem, ts);
    }
    ...
    public void onPortComponentClick(PortComponent portComponent) {
        try {
            if (lytUnassignedSourcePorts.indexOf(portComponent) > -1) {
                if (target == null && 
                    !bem.hasSpecialRelationship(portComponent.getPort().getClassName(), portComponent.getPort().getId(), "mirror", 1) && //NOI18N
    ...
    public List<BusinessObjectLight> getFreePorts() throws InventoryException {
        ...
        for (BusinessObjectLight port : thePorts) {
            if (!bem.hasSpecialRelationship(port.getClassName(), port.getId(), "mirror", 1) && //NOI18N
                !bem.hasSpecialRelationship(port.getClassName(), port.getId(), "mirrorMultiple", 1)) //NOI18N
                freePorts.add(port);
        }
        return freePorts;
    }
