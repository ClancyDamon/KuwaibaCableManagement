 ...
 */
@Component
public class DeleteScriptedQueryPoolVisualAction extends AbstractVisualAction<Dialog>  {
    /**
     ...
     */
    @Autowired
    private DeleteScriptedQueryPoolAction deleteScriptedQueryPoolAction;

    public DeleteScriptedQueryPoolVisualAction() {
    ...
    public Dialog getVisualComponent(ModuleActionParameterSet parameters) {
        ...
        if (parameters.containsKey("pool"))
            selectedPool = (ScriptedQueriesPool) parameters.get("pool");
        else 
            return null;
   
        ConfirmDialog wdwDeleteParameter = new ConfirmDialog(ts, ts.getTranslatedString("module.general.labels.confirmation"),
                String.format(ts.getTranslatedString("module.queries.confirm-delete-pool"), selectedPool.getName()),
                ts.getTranslatedString("module.general.labels.delete"));

        wdwDeleteParameter.getBtnConfirm().addClickListener((event) -> {
            try {
                deleteScriptedQueryPoolAction.getCallback().execute(new ModuleActionParameterSet(
                        new ModuleActionParameter<>("poolId", selectedPool.getId())
                ));

                fireActionCompletedEvent(new ActionCompletedListener.ActionCompletedEvent(ActionCompletedListener.ActionCompletedEvent.STATUS_SUCCESS,
                        ts.getTranslatedString(ts.getTranslatedString("module.queries.pool-deleted")), DeleteScriptAction.class));
    ...
    public AbstractAction getModuleAction() {
        return deleteScriptedQueryPoolAction;
    }
