 ...
 */
@Component
public class MoveContractToPoolAction extends AbstractAction{
    /**
    ...
    @PostConstruct
    public void init() {
        this.id = "contractman.actions.move";
        this.displayName = ts.getTranslatedString("module.contractman.actions.move-contract-to-pool.name");
        this.description = ts.getTranslatedString("module.contractman.actions.move-contract-to-pool.description");
        this.order = 1000;
        ...
        setCallback((parameters) -> {
            try {
                ...
                cms.moveContractToPool(pool.getId(), contract.getClassName(), contract.getId(), session.getUser().getUserName());
