...
public class DeleteFavoriteAction extends AbstractAction {
    ...
    @PostConstruct
    protected void init() {
        this.id = "favorites.delete-favorite";
        this.displayName = ts.getTranslatedString("module.favorites.actions.favorite-delete-favorite-name");
        this.description = ts.getTranslatedString("module.favorites.actions.favorite-delete-favorite-description");
        this.order = 1000;
        
        setCallback((parameters) -> {
            FavoritesFolder favoriteFolder = (FavoritesFolder) parameters.get("favoriteFolder");
            UserProfile user = (UserProfile) parameters.get("user");
            ActionResponse actionReesponse = new ActionResponse();
            try {
                aem.deleteFavoritesFolders(new long[]{favoriteFolder.getId()}, user.getId());
