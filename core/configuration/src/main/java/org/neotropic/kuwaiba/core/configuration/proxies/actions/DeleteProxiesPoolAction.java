 ...
 */
@Component
public class DeleteProxiesPoolAction extends AbstractAction {
    /**
    ...
    @PostConstruct
    protected void init() {
        this.id = "configman.delete-pool";
        this.displayName = ts.getTranslatedString("module.configman.proxies.actions.delete-pool.name");
        this.description = ts.getTranslatedString("module.configman.proxies.actions.delete-pool.description");
        this.order = 1000;
        
        setCallback((parameters) -> {
            InventoryObjectPool pool = (InventoryObjectPool) parameters.get("pool");
            try {
                pms.deleteProxyPool(pool.getId());
