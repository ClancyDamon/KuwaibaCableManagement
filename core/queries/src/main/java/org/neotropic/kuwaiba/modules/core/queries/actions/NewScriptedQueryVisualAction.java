 ...
 */
@Component
public class NewScriptedQueryVisualAction extends AbstractVisualAction<Dialog> {
    /**
     ...
     */
    @Autowired
    private NewScriptedQueryAction newScriptAction;
    /**
    ...
    public NewScriptedQueryVisualAction() {
    ...
    public Dialog getVisualComponent(ModuleActionParameterSet parameters) {
        if (parameters.containsKey("pool")) {
            ScriptedQueriesPool seletedPool = (ScriptedQueriesPool) parameters.get("pool");

            ConfirmDialog wdwAdd = new ConfirmDialog(ts,
                    this.newScriptAction.getDisplayName(),
            ...
            wdwAdd.getBtnConfirm().addClickListener(event -> {
                try {
                    if (txtName.getValue() == null || txtName.getValue().trim().isEmpty()) {
                        ...
                        newScriptAction.getCallback().execute(new ModuleActionParameterSet(
                                new ModuleActionParameter<>("scriptName", txtName.getValue()),
                                new ModuleActionParameter<>("poolId", seletedPool.getId()),
                                new ModuleActionParameter<>("scriptDescription", txtDescription.getValue())));

                        fireActionCompletedEvent(new ActionCompletedListener.ActionCompletedEvent(ActionCompletedListener.ActionCompletedEvent.STATUS_SUCCESS,
                                ts.getTranslatedString(ts.getTranslatedString("module.queries.actions.script-created")), NewScriptedQueryPoolAction.class));
                        wdwAdd.close();
                    }
