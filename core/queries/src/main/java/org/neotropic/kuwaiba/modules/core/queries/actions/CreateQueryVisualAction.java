 ...
 */
@Component
public class CreateQueryVisualAction extends AbstractVisualAction<Dialog> {
    /**
     ...
     */
    @Autowired
    private CreateQueryAction createQueryAction;
    /**
    ...
    public CreateQueryVisualAction() {
    ...
    public Dialog getVisualComponent(ModuleActionParameterSet parameters) {
        ConfirmDialog wdwAdd = new ConfirmDialog(ts,
                this.createQueryAction.getDisplayName(),
        ...
        wdwAdd.getBtnConfirm().addClickListener(event -> {
            try {
                if (txtName.getValue() == null || txtName.getValue().isEmpty()) {
                    ...
                    ActionResponse res = createQueryAction.getCallback().execute(new ModuleActionParameterSet(
                            new ModuleActionParameter<>("name", txtName.getValue()),
                            new ModuleActionParameter<>("structure", structure),
                            new ModuleActionParameter<>("owner", -1l),
                            new ModuleActionParameter<>("description", txtDescription.getValue())));
                    
                    fireActionCompletedEvent(new ActionCompletedListener.ActionCompletedEvent(ActionCompletedListener.ActionCompletedEvent.STATUS_SUCCESS,
                            ts.getTranslatedString(ts.getTranslatedString("module.pools.actions.pool-created")), NewScriptedQueryPoolAction.class, res));
                    wdwAdd.close();
                }
