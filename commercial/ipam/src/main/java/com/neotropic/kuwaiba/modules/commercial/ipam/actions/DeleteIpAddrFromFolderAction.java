 ...
 */
@Component
public class DeleteIpAddrFromFolderAction extends AbstractAction {
    /**
    ...
    @PostConstruct
    protected void init() {
        this.id = "ipam.removes-ip-addr";
        this.displayName = ts.getTranslatedString("module.ipam.actions.remove-ip-addr.name");
        this.description = ts.getTranslatedString("module.ipam.actions.remove-ip-addr.name.description");
        ...
        setCallback(parameters -> {
            try {
                ...
                ids.add((String) parameters.get(Constants.PROPERTY_ID));
                ipamService.deleteIpAddress(ids, false);
