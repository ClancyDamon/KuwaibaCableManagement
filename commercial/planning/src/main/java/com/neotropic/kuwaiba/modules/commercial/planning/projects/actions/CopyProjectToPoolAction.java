 ...
 */
@Component
public class CopyProjectToPoolAction extends AbstractAction {
    /**
    ...
    @PostConstruct
    protected void init() {
        this.id = "projects.copy-project";
        this.displayName = ts.getTranslatedString("module.projects.actions.copy-object-to-pool.name");
        this.description = ts.getTranslatedString("module.projects.actions.copy-object-to-pool.description");
        this.order = 1000;
        ...
        setCallback((parameters) -> {
            ...
            try {
                ps.copyProjectToPool(pool.getId(), proxy.getClassName(), proxy.getId(), session.getUser().getUserName());
