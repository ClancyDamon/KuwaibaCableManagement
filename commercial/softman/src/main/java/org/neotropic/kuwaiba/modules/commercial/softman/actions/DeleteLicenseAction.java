 ...
 */
@Component
public class DeleteLicenseAction extends AbstractAction {
    /**
    ...
    */
    ...
    @PostConstruct
    protected void init() {
        this.id = "softman.delete-license";
        this.displayName = ts.getTranslatedString("module.softman.actions.delete-license.name");
        this.description = ts.getTranslatedString("module.softman.actions.delete-license.description");
        this.order = 1000;
        
        this.setCallback((parameters) -> {
            BusinessObjectLight license = (BusinessObjectLight) parameters.get("license");
            try {
                sms.deleteLicense(license.getClassName(), license.getId());
