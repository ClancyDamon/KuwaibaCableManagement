 ...
 */
public class LayoutEditorSearch extends Div {
    
    TranslationService ts;
    
    public LayoutEditorSearch(TranslationService ts, BusinessEntityManager bem, List<BusinessObjectLight> addedNodes, List<BusinessObjectLight> addedLinks) {
        TextField txtSearch = new TextField();
        txtSearch.setWidth("300px");
        txtSearch.setValueChangeMode(ValueChangeMode.EAGER);
        txtSearch.setClearButtonVisible(true);
        txtSearch.setPlaceholder(ts.getTranslatedString("module.layout-editor.search-objects"));
                    ...
                    if (!lstEquipmentsSearch.isEmpty()) {
                        ...
                        gridEquipments.addColumn(new ComponentRenderer<>((obj) -> {
                            ...
                            btnAdd.addClickListener((clickEvent) -> {
                                ...
                                fireEvent(new NewObjectEvent(LayoutEditorSearch.this, false, obj));
                            });
    ...
    public class NewObjectEvent extends ComponentEvent<LayoutEditorSearch> {
        private final BusinessObjectLight object;
        
        public NewObjectEvent(LayoutEditorSearch source, boolean fromClient, BusinessObjectLight object) {
