 ...
 */
@Component
public class NewProxiesPoolAction extends AbstractAction {
    /**
    ...
    @PostConstruct
    protected void init() {
        this.id = "configman.new-proxies-pool";
        this.displayName = ts.getTranslatedString("module.configman.proxies.actions.new-pool.name");
        this.description = ts.getTranslatedString("module.configman.proxies.actions.new-pool.description");
        this.order = 1000;
        
        setCallback((parameters) -> {
            String name = (String) parameters.get("name");
            String desc = (String) parameters.get("desc");
            pms.createProxyPool(name, desc);
