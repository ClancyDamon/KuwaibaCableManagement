 ...
 */
package org.neotropic.kuwaiba.modules.commercial.contractman.components;
...
@Component
public class PoolContractDialog extends AbstractVisualAction<Dialog> implements ActionCompletedListener,
     ...
     */
    private Command addContractsPool;
    /**
     * action over main layout after delete pool
     */
    private Command deleteContractsPool;
    /**
    ...
    /**
     * Contains all pool inside contract
     */
    ...
    /**
     * The grid with the contracts
     */
    private Grid<BusinessObjectLight> gridContracts;
    /**
     * Contracts data provider
     */
    private ListDataProvider<BusinessObjectLight> dataProviderContracts;
    /**
     * Contracts list
     */
    private List<BusinessObjectLight> contracts;
    /**
     * Object to save the selected contracts pool
     */
    ...
    public Dialog getVisualComponent(ModuleActionParameterSet parameters) {
        ...
        addContractsPool = (Command) parameters.get("commandAddContractsPool");
        deleteContractsPool = (Command) parameters.get("commandDeleteContractsPool");
        ...
        lytContracts.setClassName("grig-pool-container");
        ...
        lytRightMain.add(lytRightHeader, lytPropertySheetPool, lytContracts);
        // End right side
    ...
    private void buildPoolsGrid() {
        try {
            ...
            listPools = cms.getContractPools();
            ...
            gridPools.addItemClickListener(event -> {
               currentPool = event.getItem();
                ...
                createContractsDataProvider(currentPool);
                showFields(true);
            });
    ...
    private void createHeaderGrid(Grid.Column column, ListDataProvider<InventoryObjectPool> dataProviderPools) {
        ...
        ActionButton btnAddPool = new ActionButton(new ActionIcon(VaadinIcon.PLUS_SQUARE_O),
                this.newContractsPoolVisualAction.getModuleAction().getDisplayName());
        btnAddPool.addClickListener((event) -> {
            this.newContractsPoolVisualAction.getVisualComponent(new ModuleActionParameterSet(
                    new ModuleActionParameter("commandClose", addContractsPool)
            )).open();
        });
    ...
    private void createContractsGrid() {
        gridContracts = new Grid<>();
        gridContracts.addThemeVariants(GridVariant.LUMO_NO_BORDER,
                GridVariant.LUMO_NO_ROW_BORDERS, GridVariant.LUMO_COMPACT);
        gridContracts.setSelectionMode(Grid.SelectionMode.NONE);
        ...
        createContractsDataProvider(null);
        Label lblVariableInPool = new Label(ts.getTranslatedString("module.contractman.contract.header"));
        HeaderRow filterRow = gridContracts.appendHeaderRow();
        filterRow.getCell(nameColumn).setComponent(lblVariableInPool);
    }
        
    private void createContractsDataProvider(InventoryObjectPool pool) {
        if (pool != null) {
            try {
                contracts = cms.getContractsInPool(pool.getId(), LIMIT);
                dataProviderContracts = new ListDataProvider<>(contracts);
                gridContracts.setDataProvider(dataProviderContracts);
                gridContracts.getDataProvider().refreshAll();
    ...
    private void createRightControlButtons() {
        ActionButton btnDeletePool = new ActionButton(new ActionIcon(VaadinIcon.TRASH),
                this.deleteContractsPoolVisualAction.getModuleAction().getDisplayName());
        btnDeletePool.addClickListener(event -> {
            this.deleteContractsPoolVisualAction.getVisualComponent(new ModuleActionParameterSet(
                    ...
                    new ModuleActionParameter("commandClose", deleteContractsPool),
    ...
    private void showFields(boolean show) {
        ...
        lytContracts.setVisible(show);
    }
        
    private void updatePropertySheet(InventoryObjectPool pool) {
        try {
            if (pool != null) {
                InventoryObjectPool aWholePool = cms.getContractPool(pool.getId(), pool.getClassName());
        ...
        if (currentPool != null) {
            try {
                ...
                if (property.getName().equals(Constants.PROPERTY_NAME)) {
                    cms.updateContractPool(currentPool.getId()
