 ...
 */
@Component
public class CopyBusinessObjectVisualAction extends AbstractVisualInventoryAction {
    /**
     ...
     */
    @Autowired
    private CopyBusinessObjectAction copyBusinessObjectAction;
    /**
    ...
    public CopyBusinessObjectVisualAction() {
    ...
    public Dialog getVisualComponent(ModuleActionParameterSet parameters) {
        ...
        if (businessObject != null) {
            ...
            ConfirmDialog wdw = new ConfirmDialog(ts
                    , String.format(ts.getTranslatedString("module.navigation.actions.copy-business-object.header"), businessObject.toString())
                    , ts.getTranslatedString("module.navigation.actions.copy-business-object.name"));
            wdw.setModal(false);
            wdw.setDraggable(true);
            wdw.setResizable(true);
            wdw.setWidth("60%");
            
            BusinessObjectSelector lytContent = new BusinessObjectSelector(ts.getTranslatedString("module.navigation.actions.copy-business-object.placeholder"), aem, bem, mem, ts);
            lytContent.addSelectedObjectChangeListener(event -> targetObject = event.getSelectedObject());
            
            wdw.getBtnConfirm().addClickListener(c -> {
                if (targetObject != null) {
                    try {
                        ...
                        for (ClassMetadataLight possibleChild : possibleChildren) {
                            if (businessObject.getClassName().equals(possibleChild.getName())) {
                                ModuleActionParameterSet actionParameters = new ModuleActionParameterSet(
                                    new ModuleActionParameter(CopyBusinessObjectAction.PARAM_TARGET_CLASS, targetObject.getClassName()),
                                    new ModuleActionParameter(CopyBusinessObjectAction.PARAM_TARGET_ID, targetObject.getId()),
                                    new ModuleActionParameter(CopyBusinessObjectAction.PARAM_OBJECT_CLASS, businessObject.getClassName()),
                                    new ModuleActionParameter(CopyBusinessObjectAction.PARAM_OBJECT_ID, businessObject.getId())
                                );
                                try {
                                    ActionResponse actionResponse = copyBusinessObjectAction.getCallback().execute(actionParameters);
                                    ...
                                    fireActionCompletedEvent(new ActionCompletedEvent(
                                        ...
                                        ts.getTranslatedString("module.navigation.actions.copy-business-object.visual.success"), 
                                        CopyBusinessObjectAction.class, 
                                        actionResponse
                                    ));
                        ...
                        new SimpleNotification(
                            ...
                            String.format(ts.getTranslatedString("module.navigation.actions.copy-business-object.is-not-possible-children"), businessObject.toString()), 
    ...
    public AbstractAction getModuleAction() {
        return copyBusinessObjectAction;
    }
}
